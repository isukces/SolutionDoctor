    ...
    static class NugetPackageAssemblyBindingChecker
    {
        ...
        private static IEnumerable<Problem> ScanProject(Project project)
        {
            ...
            foreach (var package in packageVersion)
            {
                ...
                Tuple<FileInfo, string, bool>[] versions = dlls.Select(GetDllInfo).Distinct().ToArray();
                {
                    var aa = versions.Where(q => q.Item2 == null || !q.Item3).ToArray();
                    if (aa.Any())
                    {
        ...
        private static Tuple<FileInfo, string, bool> GetDllInfo(FileInfo a)
        {
            if (!a.Exists)
                return new Tuple<FileInfo, string, bool>(a, null, false);
            try
            {
                var currentAssemblyName = AssemblyName.GetAssemblyName(a.FullName);
                return
                    new Tuple<FileInfo, string, bool>(a,
                        currentAssemblyName.Version.ToString(),
                        true);
            }
            catch
            {
                return new Tuple<FileInfo, string, bool>(a, null, true);
            }
        }
